...
| --- | --- | --- |
| Days 0–14 | Resolve [MIT license PR #32](https://github.com/safal207/Liminal-Rail-Metro/pull/32) with the owner; merge this onboarding documentation and issue templates; assign one maintainer to answer reports. | License file is present on `main`, README license statement matches it, and a fresh clone follows the published commands. Until then, label the licensing status explicitly; do not present MIT as already effective. |
