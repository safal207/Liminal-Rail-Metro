...
func TestPolicyAuthorityHardwarePolicyStillDeniesInsufficientEvidence(t *testing.T) {
	...
	if calls != 0 {
		t.Fatalf("denied policy invoked dispatcher %d times", calls)
	}
}
...
func TestPolicyAuthorityDispatcherReceivesExactBoundDescriptor(t *testing.T) {
	...
	decision, err := gate.Execute(DispatchFunc(func(op BoundOperation) error {
		...
		if op.Action.Kind != "cpu.sha256" || op.Target != "executor://cpu" || op.OperationClass != "bounded.cpu.sha256" {
			t.Fatalf("wrong bound operation: %+v", op)
		}
		return nil
	}))
	if err != nil || !decision.Allowed || called != 1 {
		t.Fatalf("bound dispatch failed: decision=%+v called=%d err=%v", decision, called, err)
	}
}
