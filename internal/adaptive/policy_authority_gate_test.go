...
func TestPolicyAuthorityRejectsMislabeledHardwareActionBeforeGate(t *testing.T) {
	...
	if cpuCalls != 0 || hardwareCalls != 0 {
		t.Fatal("rejected gate dispatched an operation")
	}
}
...
func TestPolicyAuthorityCallerCannotSupplyExecuteCallback(t *testing.T) {
	...
	if err != nil || !decision.Allowed {
		t.Fatalf("trusted dispatch failed: decision=%+v err=%v", decision, err)
	}
	if cpuCalls != 1 || hardwareCalls != 0 {
		t.Fatalf("wrong trusted handler calls cpu=%d hardware=%d", cpuCalls, hardwareCalls)
	}
}
...
func TestPolicyAuthorityHardwarePolicyStillDeniesInsufficientEvidence(t *testing.T) {
	...
	if _, err := gate.Learn(); !errors.Is(err, ErrTrustPolicyDenied) {
		t.Fatalf("expected learning denial, got %v", err)
	}
	if hardwareCalls != 0 || learningCalls != 0 {
		t.Fatalf("denied gate invoked trusted callbacks hardware=%d learning=%d", hardwareCalls, learningCalls)
	}
}
...
func TestPolicyAuthorityAllowedResultBindingUsesVerifiedGate(t *testing.T) {
	...
	if err := gate.ValidateDecisionResultBinding(bound, decision); err != nil {
