...
func TestBindPolicyAuthorityProofRefs(t *testing.T) {
	policy, err := trustpolicy.NewPolicy("external-portable-required", trustpolicy.Requirements{
	...
	manifest, err := policyauthority.NewManifest("rail-policy-authority", 1, []policyauthority.Binding{{
		OperationClass: "bounded.cpu.sha256",
		Policy:         policy,
	}})
	if err != nil {
		t.Fatal(err)
	...
	_, authorization, err := resolver.Resolve("bounded.cpu.sha256")
	if err != nil {
		t.Fatal(err)
	}
