...
func TestManifestRejectsAmbiguousExecutionContract(t *testing.T) {
	...
	if err == nil {
		t.Fatal("ambiguous execution contract unexpectedly accepted")
	}
}
...
func TestDurableResolverRejectsRollbackAndFork(t *testing.T) {
	...
	if restarted.Head().HeadHash != acceptedHead.HeadHash {
		t.Fatal("accepted head changed after exact restart")
	}
}
...
func TestRotationRequiresExplicitWeakening(t *testing.T) {
	...
	if _, err := SignRotation(current, weaker, currentKey, false); err == nil {
		t.Fatal("silent weakening unexpectedly accepted")
	}
